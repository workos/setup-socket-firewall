...
Audit schema2 separates primary observed-install `dispositions` from strict `assuranceDispositions`; retain both views in regressions. Test that bounded ordinary package-script roles under valid setup do not erase observed configuration, while scripts before valid setup, under invalid/unresolved setup or in script-only sibling jobs remain visible. Cover persistent conditional Corepack state, malformed local source after covered installs, inline/YAML environment scope, unsupported env wrappers, literal logging/prologues versus shell source/substitutions, direct executor argument boundaries and exact npm lockfile-host replacement versus actual registry overrides. `integrated` is not runtime proof; strict opacity and security findings remain separate. Do not introduce a historical cohort, coverage percentage or repository allowlist.
