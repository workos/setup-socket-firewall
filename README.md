...
Literal leading npm workspace selectors (`-w`, `--workspace`, and `--workspace=`) retain the underlying command classification: scripts stay unverified script execution, while installs and explicit `npm exec -- <target>` remain download candidates. Literal npm package selectors in `npx --package`/`-p` are recognized without treating executable payload flags as installer configuration. Dynamic selectors, unsupported options, and post-teardown executor downloads remain findings.
