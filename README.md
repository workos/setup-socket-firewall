...
| `needs-review` | An observed install interval, script-only installation-capable job, reusable call or malformed source needs investigation. |
| `integrated` | Recognized downloads have supported static setup intervals, possibly with an explicit public-fork exception; opaque execution remains unverified. |
...
Primary integration is not a rollout percentage or a certification of every command. For example, setup → `npm ci` → `npm test` can be `integrated` while strict assurance remains `needs-review`. Without setup, the same direct install remains a `needs-sfw` candidate even though the later test is opaque. A new uncovered install job cannot be masked by an integrated sibling; a script-only installation-capable sibling remains `needs-review`.
...
The small grammar recognizes direct npm/pnpm/Bun downloads, Yarn blockers, setup/teardown order and registry overrides. Each recognized download is evaluated in its own job and interval, using the approved SHA and visibility-appropriate token expression. Disabled setup provides no integration evidence; uncertain setup/install conditions, complex shell constructs and actual job containers remain unresolved. Sibling service containers alone do not invalidate host setup ordering. A job-level guard affects shared reachability rather than relative setup ordering. Ordinary run-step guards retain GitHub's implicit success gating; explicit status-function guards, conditional setup/teardown and conditional Corepack transitions remain conservative. Unresolved setup token expressions (including composite input forwarding) require review, not a claim of a wrong token; literal wrong visibility-specific secrets remain gaps.

Primary environment checks cover package-manager configuration prefixes, home/config paths, executable selection, shell startup, loader and proxy controls. Ordinary mapping keys for application credentials/settings and toolchain-version metadata do not alone invalidate observed configuration, including secret-valued expressions. Dynamic mappings, malformed keys and nonscalar values remain unresolved. The same rule applies to inline assignments: `CI=1 npm ci` and `env CI=1 npm ci` are supported; HOME/PATH/NODE_OPTIONS overrides and option-bearing `env` wrappers require review. Strict assurance retains blanket environment uncertainty. Literal Bash/sh defaults, exact `set -e` / `set -euo pipefail` prologues and literal `echo` logging do not alone invalidate primary evidence. Substitutions, imports, heredocs, redirections and unsupported shell switches are not interpreted.

Opaque scripts/actions remain strict-assurance diagnostics. Primary integration recognizes a bounded ordinary package-script role (`npm`/`pnpm`/Bun/Yarn `run` with a literal script name, or `test`/`start`/`stop`/`restart`, with literal arguments; option arguments require a `--` separator). After a valid setup interval this role is not itself an observed routing invalidation, so setup → `npm test` → `npx wrangler deploy --env production` can be `integrated`. This does **not** declare scripts safe or no-network. Script-only jobs, execution before valid setup or under unresolved/invalid setup, explicit configuration changes and teardown remain visible; a later valid install cannot hide an earlier unresolved installation-capable path. Other opaque execution before a later install can make its routing unresolved; a fresh explicit setup can re-establish configuration, but cannot erase earlier unresolved execution.

For direct `npx`/`bunx` only, a literal target (optionally preceded by `-y`/`--yes`) separates installer options from literal command arguments. Post-target `--env` or `--registry` belongs to the invoked command, not the installer's routing configuration; invoked code remains unverified. Pre-target registry overrides remain gaps; unsupported pre-target options, dynamic targets/arguments and npm-exec remain unresolved. Exact npm `--replace-registry-host=always` preserves the configured registry and is supported; other replacement modes require review. Registry/scoped/abbreviated overrides are not exempted. Corepack shim uncertainty persists across conditional/targeted controls and setup; a certain unqualified disable can reset it.

Primary integration also makes a narrow toolchain-role assumption: standard `pnpm/action-setup` (no `run_install`, or literal false) and `oven-sh/setup-bun` with supported version inputs do not invalidate routing for subsequent explicit installs. Unsupported inputs and subpaths remain unresolved. This does **not** certify their bootstrap downloads or action implementations, and they never substitute for SFW setup.

Referenced local composites have bounded nesting and cycle detection; unresolved installation-capable paths remain candidates. Missing, malformed or unresolvable local-action source retains a separate error marker and requires review even after a covered install. Public external-fork fallback is recorded per download as `fork-exception`: an explicit unprotected path, not verified protection. No historical repository cohort or exception allowlist is used.
